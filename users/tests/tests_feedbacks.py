...
class AdminFeedbacksPageTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.admin_user = User.objects.create(
            full_name='Admin User',
...
class FeedbackIntegrationTests(TestCase):
    ...
    def test_multiple_feedbacks_from_different_users(self):
        ...
        self.assertEqual(len(data['feedbacks']), 2)
        names = [f['name'] for f in data['feedbacks']]
...
class SubmitFeedbackTests(TestCase):
    ...
    def test_valid_ratings_1_to_5(self):
        ...
        for rating in range(1, 6):
            response = self.client.post(
                reverse('submit_feedback'),
                data=json.dumps({'content': f'Feedback with rating {rating}', 'rating': rating}),
                content_type='application/json'
            )
            self.assertEqual(response.status_code, 201)
            data = json.loads(response.content)
...
class GetUserFeedbacksTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create(
            full_name='Test User',
    ...
    def test_user_can_get_their_feedbacks(self):
        ...
        contents = [f['content'] for f in data['feedbacks']]
...
class FeedbacksPageTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create(
            full_name='Test User',
    ...
    def test_page_contains_feedback_form(self):
        session = self.client.session
        session['user_id'] = self.user.id
        session.save()
    ...
    def test_submit_button_visible_only_to_logged_in_users(self):
        ...
        response = self.client.get(reverse('feedbacks_page'))
        content = response.content.decode('utf-8')
        self.assertNotIn('שלח משוב', content)


class ViewAllFeedbacksTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user1 = User.objects.create(
            full_name='User One',
    ...
    def test_multiple_feedbacks_display_correctly(self):
        ...
        self.assertEqual(data['feedbacks'][0]['rating'], 1)
        self.assertEqual(data['feedbacks'][4]['rating'], 5)
...
class AllFeedbacksPageTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create(
            full_name='Test User',
    ...
    def test_add_feedback_button_visible_only_to_logged_in_users(self):
        # Check as authenticated user
        session = self.client.session
        session['user_id'] = self.user.id
        session.save()
...
class FeedbackIntegrationTests(TestCase):
    ...
    def test_complete_feedback_submission_flow(self):
        ...
        # Verify it appears in user feedbacks
        response = self.client.get(reverse('get_user_feedbacks'))
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_multiple_users_feedbacks_are_separate(self):
        ...
        # Verify user1 only sees their own feedback
        session = self.client.session
        session['user_id'] = self.user.id
        session.save()
        ...
        # Verify user2 only sees their own feedback
        session = self.client.session
        session['user_id'] = user2.id
        session.save()
    ...
    def test_all_feedbacks_visible_to_everyone(self):
        ...
        # Submit feedbacks from two users
        feedback1 = Feedback.objects.create(
            user=self.user,
            content='User 1 feedback',
            rating=5
        ...
        # Unauthenticated user can see all feedbacks
        response = self.client.get(reverse('view_all_feedbacks'))
        data = json.loads(response.content)
        self.assertEqual(len(data['feedbacks']), 2)

        # Authenticated user (user1) can see all feedbacks
        session = self.client.session
        session['user_id'] = self.user.id
        session.save()
        ...
        # Authenticated user (user2) can see all feedbacks
        session = self.client.session
        session['user_id'] = user2.id
        session.save()
    ...
    def test_public_feedbacks_match_submitted_feedbacks(self):
        ...
        # Guest user fetches all feedbacks
        self.client.session.flush()
        response = self.client.get(reverse('view_all_feedbacks'))
        data = json.loads(response.content)
